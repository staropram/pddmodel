...
png(filename = "figures/reoffending_vs_pf_original.png", width = 600, height = 600)
...
# save it to PNG
savedPlot <- recordPlot()
png(filename = "figures/rehab_vs_pf_original.png", width = 600, height = 600)
replayPlot(savedPlot)
invisible(dev.off())
```
...
```{r,echo=F,fig.width=8,fig.height=8}
plot_hosp_power_vs_pf_sd()
# save it to PNG
savedPlot <- recordPlot()
png(filename = "figures/hospital_vs_pf_original_both.png", width = 600, height = 600)
replayPlot(savedPlot)
invisible(dev.off())
```
...
# save it to PNG
savedPlot <- recordPlot()
png(filename = "figures/hospital_vs_pf_original_g1.png", width = 600, height = 600)
replayPlot(savedPlot)
invisible(dev.off())
```
...
# save it to PNG
savedPlot <- recordPlot()
png(filename = "figures/hospital_vs_pf_original_g2.png", width = 600, height = 600)
replayPlot(savedPlot)
invisible(dev.off())
```
