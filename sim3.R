...
configList <- lapply(impactsToTest,function(impact) {
   ...
   # assume 10% police force variability of baseline rate
   currentConfig <- modify_police_force_variability(currentConfig,0.10)
...
# run the metasim
dryRun <- F
simOutputs <- meta_sim(configList,"Impacts",forceRerun=F,dryRun=dryRun)
...
# render the Rmd
render("sim3.Rmd",output_dir="outputs")
